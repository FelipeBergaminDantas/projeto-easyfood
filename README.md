...
- **UniFECAF** - Instituição de ensino
- **Professor** - Orientação sobre arquitetura de software e melhores práticas
- **Comunidade Open Source** - Ferramentas e bibliotecas utilizadas (Node.js, Express, Prisma, PostgreSQL)
...
1. **Verifique a documentação**:
   ...
   - [REVIEW_GUIDE.md](REVIEW_GUIDE.md) - Guia de revisão completo
   - [EXAMPLES.md](EXAMPLES.md) - Exemplos de uso

2. **Problemas comuns**:
   - **Erro de conexão**: Verifique se o PostgreSQL está rodando
   - **Erro no Prisma**: Execute `npm run db:setup` novamente
   - **Porta em uso**: Altere a `PORT` no arquivo `.env`
   - **Token inválido**: Gere um novo JWT_SECRET forte
